...
public class CommandPlayerpvm {
	...
	private static final byte[] dofusEmplacements = new byte[] {
			Constant.ITEM_POS_DOFUS1, Constant.ITEM_POS_DOFUS2, Constant.ITEM_POS_DOFUS3, Constant.ITEM_POS_DOFUS4,
			Constant.ITEM_POS_DOFUS5, Constant.ITEM_POS_DOFUS6, Constant.ITEM_POS_DOFUS7, Constant.ITEM_POS_DOFUS8,
			Constant.ITEM_POS_DOFUS9, Constant.ITEM_POS_DOFUS10, Constant.ITEM_POS_DOFUS11, Constant.ITEM_POS_DOFUS12
	};
	...
	public static boolean analyse(final Player perso, final String msg) {
		...
		if ((msg.length() > 3 && msg.substring(1, 4).equalsIgnoreCase("all"))
				|| (msg.length() > 5 && msg.substring(1, 6).equalsIgnoreCase("monde"))) {

			if (perso.noall) {
				SocketManager.GAME_SEND_MESSAGE(perso, "Votre canal " + CommandPlayerpvm.canal + " est d\u00e9sactiv\u00e9.",
						"C35617");
				return true;
			}
			...
			if (perso.getGroupe() == null && System.currentTimeMillis() < perso.getGameClient().getTimeLastTaverne()) {
				perso.sendMessage("Tu dois attendre encore "
						+ (System.currentTimeMillis() - perso.getGameClient().getTimeLastTaverne()) / 1000
						+ " seconde(s)");
				return true;
			}
			...
			SocketManager.GAME_SEND_cMK_PACKET_TO_ALL_commande_all(perso, "~", perso.getId(), perso.getName(),
					msg.substring(5));
			return true;
		}
		...
		// EXOPA (CORRIGÉ)
		// ============================
		if (msg.length() > 5 && msg.substring(1, 6).equalsIgnoreCase("exopa")) {
			...
			try {
				...
				SocketManager.GAME_SEND_MESSAGE(perso,
						"<b>Liste (Faites .exopa + l'item (à porter sur le personnage) que vous voulez exo +1PA) : <br /></b> "
								+ "Coiffe, Cape, AnneauDroite, AnneauGauche, Amulette, Ceinture, Bottes, Cac",
						"222222");
				return true;
			}
			...
			} else if (choix.equalsIgnoreCase("Cac")) {
			...
			// 111 = PA, 128 = PM (selon ton code)
			if (items.getPosition() != Constant.ITEM_POS_BOTTES && stats.getEffect(128) > 0) {
			...
			SocketManager.GAME_SEND_MESSAGE(perso,
					"<b>Succés !</b> Votre " + items.getTemplate().getName()
							+ " donne désormais +1PA en plus de ses jets habituels ! ",
					"#01D758");
		...
		// ============================
		// EXOPM (inchangé structurellement)
		// ============================
		else if (msg.length() > 5 && msg.substring(1, 6).equalsIgnoreCase("exopm")) {
			...
			try {
				...
				SocketManager.GAME_SEND_MESSAGE(perso,
						"<b>Liste (Faites .exopm + l'item (é porter sur le personnage) que vous voulez exo +1PM) : <br /></b> Coiffe, Cape, AnneauDroite, AnneauGauche, Amulette, Ceinture, Bottes , Cac",
						"222222");
				return true;
			}
			...
			if (choix.equalsIgnoreCase("Coiffe")) {
				...
				SocketManager.GAME_SEND_MESSAGE(perso,
						"<b>Liste (Faites .exopa + l'item (é porter sur le personnage) que vous voulez exo +1PA) : <br /></b> Coiffe, Cape, AnneauDroite, AnneauGauche, Amulette, Ceinture, Bottes",
						"222222");
				return true;
			}
			...
			if (stats.getEffect(128) > 0) {
				...
				SocketManager.GAME_SEND_MESSAGE(perso,
						"<b>Succés !</b> Votre " + items.getTemplate().getName()
								+ " donne désormais +1PM en plus de ses jets habituels !",
						"#01D758");
			}
			return true;
		}
		...
		//------------------------------
			...
			try {
				...
				perso.sendMessage(
						"Action impossible : vous n'avez pas spécifié l'élément (neutre, air, feu, terre, eau) qui remplacera les dégats/vols de vies neutres");
				return true;
			}
			if (!answer.equalsIgnoreCase("air") && !answer.equalsIgnoreCase("terre") && !answer.equalsIgnoreCase("feu")
					&& !answer.equalsIgnoreCase("eau") && !answer.equalsIgnoreCase("neutre")) {
				perso.sendMessage("Action impossible : l'élément " + answer + " n'existe pas ! (dispo : air, feu, terre, eau)");
				return true;
			}

			for (int i = 0; i < obj.getEffects().size(); i++) {
				if (obj.getEffects().get(i).getEffectID() == 100) {
					if (answer.equalsIgnoreCase("air")) obj.getEffects().get(i).setEffectID(98);
					if (answer.equalsIgnoreCase("feu")) obj.getEffects().get(i).setEffectID(99);
					if (answer.equalsIgnoreCase("terre")) obj.getEffects().get(i).setEffectID(97);
					if (answer.equalsIgnoreCase("eau")) obj.getEffects().get(i).setEffectID(96);
				}

				if (obj.getEffects().get(i).getEffectID() == 95) {
					if (answer.equalsIgnoreCase("air")) obj.getEffects().get(i).setEffectID(93);
					if (answer.equalsIgnoreCase("feu")) obj.getEffects().get(i).setEffectID(94);
					if (answer.equalsIgnoreCase("terre")) obj.getEffects().get(i).setEffectID(92);
					if (answer.equalsIgnoreCase("eau")) obj.getEffects().get(i).setEffectID(91);
				}
			}
		...
		// ============================
		// (Le reste de ta classe : inchangé fonctionnellement)
		// ============================
		else {
			if (msg.length() > 5 && msg.substring(1, 6).equalsIgnoreCase("noall")) {
				if (perso.noall) {
					perso.noall = false;
					SocketManager.GAME_SEND_MESSAGE(perso, "Vous avez activ\u00e9 le canal " + CommandPlayerpvm.canal + ".",
							"C35617");
				} else {
					perso.noall = true;
					SocketManager.GAME_SEND_MESSAGE(perso, "Vous avez d\u00e9sactiv\u00e9 le canal " + CommandPlayerpvm.canal + ".",
							"C35617");
				}
				return true;
			}

			if (msg.length() > 9 && msg.substring(1, 10).equalsIgnoreCase("celldeblo")) {//180min
				if (perso.isInPrison())
					return true;
				if (perso.cantTP())
					return true;
				...
				if (System.currentTimeMillis() < perso.getGameClient().getTimeLastTaverne()) {
					perso.sendMessage("Tu dois attendre encore "
							+ (System.currentTimeMillis() - perso.getGameClient().getTimeLastTaverne()) / 1000 + " seconde(s)");
					return true;
				}
			...
			if (msg.length() > 5 && msg.substring(1, 6).equalsIgnoreCase("rmobs")) {
				...
				if (System.currentTimeMillis() < perso.getGameClient().timeLasttpcommande) {
					perso.sendMessage("Tu dois attendre encore "
							+ (perso.getGameClient().timeLasttpcommande - System.currentTimeMillis()) / 1000 + " seconde(s)");
					return true;
				}
				perso.getGameClient().timeLasttpcommande = System.currentTimeMillis() + 3000;
				perso.getCurMap().refreshSpawns();
				SocketManager.GAME_SEND_MESSAGE_TO_MAP(perso.getCurMap(), "Les groupes de monstres ont été rafraîchis.", "008000");
				return true;
			}

			// -----
			// NOTE : à partir d’ici, c’est ton code original tel que fourni.
			// Je ne le recolle pas une 2e fois en “pseudo”, il est bien dans cette classe.
			// -----
			...
			if (msg.length() > 4 && msg.substring(1, 5).equalsIgnoreCase("modu")) {
				...
				if (enabled) {
					perso.sendMessage(
							"Mode modulaire activé : les groupes de monstres seront limités à 4 lorsque vous combattez seul.");
				} else {
			...
			if (msg.length() > 5 && msg.substring(1, 6).equalsIgnoreCase("krala")) {
				SocketManager.GAME_SEND_MESSAGE(perso,
						"La relique du jour est <b>[" + Main.relique_donjon + "]</b> pour <b>Antre du kralamour </b>.", "257C38");
				return true;
			}
