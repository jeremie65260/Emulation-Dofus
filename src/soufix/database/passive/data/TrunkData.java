...
public class TrunkData extends AbstractDAO<Trunk>
{
  ...
  public int load()
  {
    ...
    try
    {
      ...
      while(RS.next())
      {
        Main.world.addTrunk(new Trunk(RS.getInt("id"),RS.getInt("id_house"),RS.getShort("mapid"),RS.getInt("cellid")));
        nbr++;
      }
    }
  ...
  public void insert(Trunk trunk)
  {
    ...
    try
    {
      p=getPreparedStatement("INSERT INTO `coffres` (`id`, `id_house`, `mapid`, `cellid`) "+"VALUES (?, ?, ?, ?)");
