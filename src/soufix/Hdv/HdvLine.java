...
public class HdvLine
{
  ...
  private ArrayList<ArrayList<HdvEntry>> entries=new ArrayList<ArrayList<HdvEntry>>(3); //La premiére ArrayList est un tableau de 3 (0=1 1=10 2=100 de quantité)
  ...
  {
    return this.getStrStats().equalsIgnoreCase(toAdd.getGameObject().parseStatsStringSansUserObvi())&&toAdd.getGameObject().getTemplate().getType()!=85;//Récupére les stats de l'objet et compare avec ceux de la ligne
  }
  ...
  public boolean addEntry(HdvEntry toAdd)
  {
    ...
    this.sort(index);
    return true;//Anonce que l'objet é été accepté
  }
  ...
  public int[] getFirsts()
  {
    ...
    for(int i=0;i<this.getEntries().size();i++)
    {
      try
      {
        toReturn[i]=this.getEntries().get(i).get(0).getPrice();//Récupére le premier objet de chaque liste
      }
  ...
  {
    int totalSize=this.getEntries().get(0).size()+this.getEntries().get(1).size()+this.getEntries().get(2).size();//Additionne le nombre d'objet de chaque quantité
