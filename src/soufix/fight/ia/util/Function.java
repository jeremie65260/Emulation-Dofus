...
public class Function
{
  ...
  public boolean testCotes(int cellWeAre, int cellWego)//Nous permet d'interdire le déplacement du bord vers des cellules hors map
  {
  ...
  private ArrayList<GameCase> filterPathToAvoidOwnTraps(Fight fight, Fighter mover, List<GameCase> rawPath)
  {
    ArrayList<GameCase> safePath=new ArrayList<>();
  ...
  public String getBestTargetZone2(Fight fight, Fighter fighter, SortStats spell, int launchCell, boolean line)
  {
    if(fight==null||fighter==null)
      return "";
 ...
 public String moveToAttackIfPossible2(Fight fight, Fighter fighter)
    {
        try {
            ...
            for (int i : cells) {
                for (SortStats S : sorts) {
                    String targetVal = getBestTargetZone2(fight, fighter, S, i, false);
...
public static Fighter getNearestPlayer(Fight fight, Fighter fighter, int minRange, int maxRange) {
        if (fight == null || fighter == null)
            return null;
        int dist = 1000;
        Fighter curPlayer = null;
        for (Fighter player : fight.getFighters(1)) {
            if (player.isDead())
                continue;
            if (player.getTeam2() == fighter.getTeam2()) { // Si c'est un joueur allié
                int d = PathFinding.getDistanceBetween(fight.getMap(), fighter.getCell().getId(), player.getCell().getId());
                if (d >= minRange && d <= maxRange && d < dist) {
                    dist = d;
                    curPlayer = player;
                }
            }
