...
public void setTimeLastTaverne(long timeLastTaverne) {
    ...
    else
    {
      ...
      try
      {
        ...
        if (npcTemplate.getId() == 15024) {
                        ...
                        final int value = isMountCertificate ? 1250 * qua : template.getPrice() * qua;
      ...
      for(Pair<Integer, Integer> couple : ((BreakingObject)value).getObjects())
      {
        ...
        for(int k=couple.getRight();k>0;k--)
        {
          ...
          if(type>23&&type!=81&&type!=82)
        	  continue;
           calcule ++;
          for(Entry<Integer, Integer> entry1 : object.getStats().getMap().entrySet())
          {
  ...
  private void askOfflineExchange()
  {
    ...
    for(Entry<Integer, Integer> entry : this.player.getStoreItems().entrySet())
    {
      if(entry.getValue()<=0)
      {
        this.disconnect();
    ...
    switch(packet.charAt(2))
    {
      ...
      case '2'://Npc Exchange
        ...
        if(this.player.getCurMap().getNpc(id)!=null)
        {
          ...
          if (npcTemplate.getId() == 15024) {
            SocketManager.GAME_SEND_MESSAGE(this.player,"les montures coutent 1250 gladiatons.");
          }
        }
        break;
      case '4'://StorePlayer
  ...
  { 
    ...
    try
    {
      ...
      else
      {
        ...
        if(this.player.getCurMap()!=null&&Constant.isGladiatroolMap(this.player.getCurMap().getId())&&position!=Constant.ITEM_POS_NO_EQUIPED&&object.getTemplate().getType()!=Constant.ITEM_TYPE_OBJET_VIVANT)
        {
