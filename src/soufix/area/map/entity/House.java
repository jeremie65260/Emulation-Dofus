...
public class House
{
      ...
      if(house.getValue().getGuildId()>0) //Maison de guilde
      {
        ...
        if(G!=null)
        {
          ...
          else
          {
            ...
            else if(house.getValue().canDo(Constant.H_OBLASON))//Pas de guilde/guilde-differente
            {
              packet.append(";").append(Gname).append(";").append(Gemblem);
            }
          }
  ...
  public static void buy(Player player)//Acheter une maison
  {
    ...
    for(Trunk trunk : Trunk.getTrunksByHouse(house))
    {
      if(house.getOwnerId()>0)
        trunk.moveTrunkToBank(Main.world.getAccount(house.getOwnerId()));//Deplacement des items vers la banque
    ...
    if(house.getOwnerId()>0)
    {
      ...
      if(seller.getCurrentPlayer()!=null)//FIXME: change the packet (Im)
        SocketManager.GAME_SEND_MESSAGE(seller.getCurrentPlayer(),"Une maison que vous possediez a ete vendue "+house.getSale()+" kamas.");
  ...
  public static void sell(Player P, String packet)//Vendre une maison
  {
    ...
    if(h.isHouse(P,h))
    {
      ...
      //Rafraichir la map apres la mise en vente
  ...
  {//Entrer dans la maison
    if(P.getFight()!=null||P.getExchangeAction()!=null)
      return;
    if(this.getOwnerId()==P.getAccID()||(P.get_guild()!=null&&P.get_guild().getId()==this.getGuildId()&&canDo(Constant.H_GNOCODE)))//C'est sa maison ou meme guilde + droits entrer sans pass
      open(P,"-",true);
    else if(this.getOwnerId()>0) //Une personne autre la acheter, il faut le code pour rentrer
      SocketManager.GAME_SEND_KODE(P,"CK0|8");//8 etant le nombre de chiffre du code
...
public void setSale_base(int sale_base) {
  ...
  public void parseIntToRight(int total)
  {
    ...
    if(haveRight.size()>0) //Si les droits contiennent quelque chose -> Vidage (Meme si le HashMap supprimerais les entrees doublon lors de l'ajout)
      haveRight.clear();

    initRight(); //Remplissage des droits

    Integer[] mapKey=haveRight.keySet().toArray(new Integer[haveRight.size()]); //Recupere les clef de map dans un tableau d'Integer
